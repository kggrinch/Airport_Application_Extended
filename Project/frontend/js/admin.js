$(document).ready(function()
{
    ...
    function createCard(data)
    {
        ...
        var cardBody = $(`<div class="card-body text-center">
            ...
            <p class="card-text">Gate: ${data.gate}</p>
                <a href="view.html?from=admin&id=${data.id}" class="btn btn-outline-dark rounded-4 text-white" data-flight-id="${data.id}">View</a>
                <a href="edit.html?id=${data.id}" class="btn btn-outline-dark rounded-4 text-white" data-flight-id="${data.id}">Edit</a>
                <button class="btn btn-outline-danger delete-btn" data-flight-id="${data.id}">Delete</button>
            </div>`);
    ...
    {
        $('#data-container .row').empty(); // clear pervious cards
    ...
    // Get request to backend endpoint to retrieve all flights via [admin web service]
    function fetchData()
    {
        $.ajax
        ({
    ...
    // Delete request to backend endpoint to delete flight given id via [admin web service]
    // flight_to_delete - flight id of flight to delete
    function delete_flight(flight_to_delete)
    {
        $.ajax
        ({
            url: `http://localhost:3000/admin/${flight_to_delete}`,
            method: "DELETE",
            dataType: "json",
            success: function(response)
            {
                alert(`Flight: ${flight_to_delete} deleted successfully`);
                fetchData(); // refresh cards upon success
            },
    ...
    function search_filter(search_type, value, card)
    {
        card.filter(function()
        {
            ...
            $(this).toggle(category.indexOf(value) > -1); // toggle card if value matches data search-type
        });
    }
    ...
    // delete button on card handler
    // calls delete function
    $(document).on(`click`, `.delete-btn`, function(event)
    {
        flight_to_delete = event.target.dataset.flightId;
        var message = `Click OK to Confirm Deletion of ${flight_to_delete}`
        if(!confirm(message)) return
        delete_flight(flight_to_delete);
    });
    ...
    {
        $(`#searchInput`).val(``); // clear search input form
        selected_search_option = $(`.form-select`).val(); // update selected search option
        
        // Update placeholder inside search form based on selected search option
