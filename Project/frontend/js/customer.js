    ...
    function createCard(data)
    {
        ...
        var cardBody = $(`<div class="card-body text-center">
            ...
            <p class="card-text">Gate: ${data.gate_number}</p>
                <a href="view.html?user_id=${user_id}&from=view&flight_id=${data.flight_number}" class="btn btn-outline-dark rounded-4 text-white" data-flight-id="${data.id}">View</a>
                <a href="seat.html?user_id=${user_id}&from=view&flight_id=${data.flight_number}" class="btn btn-outline-info rounded-4 text-white" data-flight-id="${data.id}">Seat Selection</a> 
            </div>`);
